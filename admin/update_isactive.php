...
try {
    ...
    $stmt = $pdo->prepare("UPDATE bits_sys_modify SET isActive = 0 WHERE upload_dir LIKE '%uploads/bg-%'");
    $stmt->execute();
    
    $stmt = $pdo->prepare("UPDATE bits_sys_modify SET isActive = 1 WHERE upload_dir = :upload_dir AND upload_dir LIKE '%uploads/bg-%'");
