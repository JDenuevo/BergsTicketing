...
while ($row5 = $result1->fetch_assoc()) {
    $upload_sl = '../admin/' . $row5['upload_dir'];
    $title = "BERGS";
}
...
// Add email from session to the WHERE clause
...
if (isset($_GET['date'])) {
    ...
    switch ($dateType) {
        case '30days':
            $sql .= " AND date_created >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)";
            break;
        case '7days':
            $sql .= " AND date_created >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)";
            break;
        case 'yesterday':
            $sql .= " AND DATE(date_created) = DATE(DATE_SUB(NOW(), INTERVAL 1 DAY))";
            break;
        case 'today':
    ...
    $dateTo = $_GET['dateto'];
    $sql .= " AND date_created BETWEEN '$dateFrom' AND '$dateTo'";
} else {
