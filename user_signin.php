...
if (isset($_POST['username']) && isset($_POST['password'])) {
    ...
    $sql = "SELECT * FROM bits_login WHERE username='$uname'";
    if ($rs = $conn->query($sql)) {
        if ($rs->num_rows > 0) {
            ...
            if ($row['password'] === $pass && $row['privilege'] === $privilege) {
                ...
                if ($row['privilege'] === "Administrator") {
                    ...
                    exit();
                } else if ($row['privilege'] === "End-User") {
    ...
    if (!empty($errors)) {
        ...
        header('Location: signing_in.php?errors=' . urlencode($errorString));
        exit();
    }
} else {
